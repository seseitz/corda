...
class SandboxExecutorTest : TestBase() {
    ...
    fun `cannot execute runnable that catches ThresholdViolationException`() = sandbox(DEFAULT) {
        TestCatchThresholdViolationException().apply {
            assertThat(apply(0)).isEqualTo(1)
        }
        ...
        assertThatExceptionOfType(SandboxException::class.java)
                ...
                .withCauseExactlyInstanceOf(ThresholdViolationException::class.java)
    ...
    class TestCatchThresholdViolationException : Function<Int, Int> {
        override fun apply(input: Int): Int {
            return try {
                throw ThresholdViolationException("Can't catch this!")
    ...
    @Test
    fun `cannot execute runnable that catches RuleViolationException`() = sandbox(DEFAULT) {
        TestCatchRuleViolationException().apply {
            assertThat(apply(0)).isEqualTo(1)
        }

        val contractExecutor = DeterministicSandboxExecutor<Int, Int>(configuration)
        assertThatExceptionOfType(SandboxException::class.java)
                .isThrownBy { contractExecutor.run<TestCatchRuleViolationException>(0) }
                .withCauseExactlyInstanceOf(RuleViolationException::class.java)
                .withMessageContaining("Can't catch this!")
    }

    class TestCatchRuleViolationException : Function<Int, Int> {
        override fun apply(input: Int): Int {
            return try {
                throw RuleViolationException("Can't catch this!")
            } catch (exception: RuleViolationException) {
                1
            }
        }
    }
